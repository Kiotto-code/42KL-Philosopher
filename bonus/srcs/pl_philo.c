...
void *motherfucker(void *temp)
{
	...
	while (1)
	{
		...
		pl_show(pl_time() - record->starttime, pl, FORK, record->printer);
		pthread_mutex_lock(pl->r_fork);
		pl_show(pl_time() - record->starttime, pl, FORK, record->printer);
		pl_show(pl_time() - record->starttime, pl, EAT, record->printer);
		pl->last_meal = pl_time();
		...
		// pl->num_meals += 1;
		// pl->num_meals += 1;
		// if (pl->num_meals >= record->meal_target && record->meal_target > 0)
		// if (pl->num_meals >= pl->record->meal_target && pl->record->meal_target > 0)
		// 	pl->fulfilled = 1;
	
		// if (pl->record->end == 1)
		// 	pl_usleep(pl->record->time_to_die);
		// if (pl_check_full() == 1)
		// 	break;
		pl_show(pl_time() - record->starttime, pl, SLEEP, record->printer);
		pl_usleep(record->time_to_sleep);
		pl_show(pl_time() - record->starttime, pl, THINK, record->printer);
	}
	return (NULL);
}
...
int pl_run_trd(t_thread *pl, t_book *record)
{
	...
	if (pthread_create(&checker, NULL, pl_checker, (void *)pl) != 0)
		return (err_display("PTHREAD ERROR"));
